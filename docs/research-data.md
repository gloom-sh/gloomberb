...
The optional IV overlay reuses the OVDV chain loader and its quote cleaning. It is a single dated current ATM observation with an explicit option tenor, never a historical implied-volatility line. Missing or stale option data do not prevent realized-volatility analysis. Historical IV, IV rank and IV percentile come from the stored history in `HIVG` (see [Implied volatility history](#implied-volatility-history)). The chart composer's Realized vol indicator uses the same estimators, with persisted window and estimator settings and 252-session annualization on source daily price observations. A synthetic live quote appended to the price chart does not become an extra daily observation for volatility. Invalid windows remain gaps through their complete warmup; intraday bars are not treated as daily sessions. OMON HV30 uses this same close-to-close implementation.

Put-call parity estimates the forward as `K + exp(rT) (Cmid - Pmid)` from paired two-sided quotes on the nearest two strikes on each side of spot. The median reduces sensitivity to an inconsistent pair. A pair whose forward is more than 5% plus 100% a year of carry (in log terms) away from spot grown at the Treasury rate is discarded: stale quotes left on far strikes, such as contracts listed before a split, otherwise imply forwards several times spot. When every pair is discarded the forward is unavailable. The result preserves the contributing contracts, warns when the strikes do not bracket spot or quote intervals disagree, and is unavailable if no pair exists. Implied continuous dividend yield is `r - ln(F/S)/T`; it can include borrow effects and American-exercise differences. An unpaired quote does not establish a forward. Delta and strike inversion use the existing European pricer and signed spot delta, including continuous dividend carry. Forward pricing through that pricer sets its underlying to F and its dividend yield equal to r, so discounting is applied exactly once.
...
`CRYP` requests one Gloom Cloud payload for twelve active Alpaca US USD pairs. The latest trade retains its own timestamp; a newer quote or retrieval does not redate it. A trade older than thirty minutes is stale and a trade older than twenty-four hours cannot populate the current price. Cached prices continue to age locally. The footer uses the newest usable trade and identifies stale or unavailable prices rather than dating the whole board by its oldest pair. Failed refreshes retain dated observations; an absent endpoint has an explicit unavailable state.

UTC DAY compares the latest trade with the exact prior UTC close for that trade date. It is not a rolling twenty-four-hour return. 7D CLOSED compares the latest completed UTC daily close with the close exactly seven calendar days earlier, requiring all eight observations. BASE VOL is the completed UTC day's traded base-asset quantity on Alpaca US, for example BTC rather than USD. Its date is shown above the board and its base unit beside each value. It is not total crypto-market volume or rolling twenty-four-hour volume.

Alpaca bars include trades and quote midpoint prices. A day with no trades can have a valid quoted close and zero volume; these observations remain explicitly quote-only in reports and the detail. Price percentiles compare the current trade with completed daily bar closes from the same source, not solely executed closing trades or executable prices. Detail charts use the returned daily observations through the shared composer. Missing days remain null chart gaps; the compact sparkline connects observed closes. Generic composer expressions such as `BTC-USD:price` can use a different Cloud history provider and must not be assumed to share Alpaca volume or price bases.
