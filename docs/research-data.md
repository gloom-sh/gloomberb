...
`CRYP` lists the largest crypto assets by market cap: up to 100 coins, with stablecoins on their own tab. Wrapped, staked and bridged copies of a larger coin, and pairs with under $1M of 24h volume, are left out so each market appears once. The universe, 24h volume, supply, 52-week range and the last 30 completed daily closes come from one Gloom Cloud payload refreshed every minute.

Prices stream like any other quote. The rows on screen subscribe to the shared market socket, and each live quote moves the price, day change, market cap (circulating supply times price), 7D, 30D and 1Y returns and the end of the sparkline. The footer shows whether the feed is live, mixed or polling, and the time of the newest price. With live streaming off in the pane settings, quotes refresh once a minute.
