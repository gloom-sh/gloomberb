...
test("a ticker without a 10-K report shows the empty state, not a load error",async()=>{
 transport(()=>Response.json({message:"No risk factor reports for this ticker"},{status:404}));await mount();const empty=capture("list-404");expect(empty).toContain("No 10-K risk factors on file for CONTROL.");expect(empty).not.toContain("Report list:");expect(empty).not.toContain("Could not load risk reports");expect(requests).toEqual(["/public/risks/CONTROL"]);
});
