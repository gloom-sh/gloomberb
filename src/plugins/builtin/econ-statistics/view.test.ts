...
test("1Y ago matches calendar periods even when an intermediate month is missing", () => {
  ...
  const view = projectStat({ stat: resolveStatArg("cpi-yoy")!, points, trend: fitTrend(points) }, "5Y");
  ...
  expect(projectStat({ stat: resolveStatArg("cpi-yoy")!, points: missingBase, trend: fitTrend(missingBase) }, "5Y").yearAgo).toBeNull();
});
...
test("daily 1Y ago clamps February 29 to February 28 instead of looking forward into March", () => {
  ...
  const view = projectStat({ stat: resolveStatArg("ten-year")!, points, trend: fitTrend(points) }, "5Y");
...
test("an on-schedule print is not stale until its release is overdue", () => {
  const stale = (id: string, latest: string, now: string, months = 1) => {
    ...
    return projectStat({ stat: resolveStatArg(id)!, points, trend: fitTrend(points) }, "5Y", { nowMs: Date.parse(`${now}T12:00:00Z`) }).observationStale;
  };
