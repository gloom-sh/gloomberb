...
test("fallback preserves reported settlement metrics without inventing dated denominators", async () => {
  ...
  const { records: rows } = await loadShortInterest("TEST", fallback);
...
test("reported zero and percentages above 100 remain source data", async () => {
  ...
  expect((await loadShortInterest("TEST", fallback)).records[1]).toMatchObject({ sharesShort: 0, shortRatio: 0, averageDailyVolume: null, shortPercentFloat: 0 });
  yahoo.mockResolvedValue(record(150_000_000, 2, 1.5));
  expect((await loadShortInterest("TEST", fallback)).records[1]!.shortPercentFloat).toBe(150);
});

test("FINRA average daily volume remains independent of its reported days-to-cover ratio", async () => {
  const { records: rows } = await loadShortInterest("TEST", { getCloudShortInterest: async () => ({ status: "success", data: {
